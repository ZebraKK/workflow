...
func (s *Stage) parallelRun(input string, rcder *record.Record, logger Logger) error {
	if rcder == nil {
...
func (s *Stage) worker(stp steper, input string, rcder *record.Record, logger Logger) <-chan error {
...
// 异步处理，也可能并发的来
// 任务状态不能直接更新
func (s *Stage) parallelAsyncHandler(resp string, runningID string, ids []int, stageIndex int, rcder *record.Record, logger Logger) {
	...
	stp.AsyncHandler(resp, runningID, ids, stageIndex+1, nextRcrd, stepLogger)
